...
export const getUpdateMovies = async ({
  resource = "phim",
  id,
  values,
}: Props) => {
...
export const getListCategoryChair = async ({
  resource = "loai-ghe",
}: Props) => {
  const { data } = await axiosClient.get(resource);
  return data;
...
export const getDeleteCategoryChair = async ({
  resource = "loai-ghe",
  id,
}: Props) => {
  if (!id) return;
...
export const getCreateCategoryChair = async ({
  resource = "loai-ghe",
  values,
}: Props) => {
  const { data } = await axiosClient.post(resource, values);
...
export const getUpdateCategoryChair = async ({
  resource = "loai-ghe",
  id,
  values,
}: Props) => {
