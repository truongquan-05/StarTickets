...
const resourceClient = "client/do_an";
...
export const useListFoodsClient = () => {
  return useQuery({
    queryKey: [resourceClient],
    queryFn: () => getListFoodsClient({ resource: resourceClient }), // <-- truyền đúng dạng object
  });
};
...
export const useCreateFood = () => {
  ...
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (values: Omit<Food, "id">) =>
      getCreateFood({ resource, values }),
    onSuccess: () => {
...
export const useUpdateFood = () => {
  ...
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, values }: { id: number; values: Partial<Food> }) =>
      getUpdateFood({ resource, id, values }),
    onSuccess: () => {
