...
import { useCreateVaiTro, useDeleteVaiTro, useListVaiTro, useUpdateVaiTro } from '../../hook/hungHook';
...
const VaiTro = () => {
  ...
  const [editingItem, setEditingItem] = useState<IVaiTro | undefined>(undefined);
  ...
  const { mutate: createCategoryChair } = useCreateVaiTro({ resource: "vai_tro" });
  const { mutate: updateCategoryChair } = useUpdateVaiTro({ resource: "vai_tro" });
  ...
  return (
    <Card title="Danh sách vai trò" bordered={true} style={{ margin: 10, boxShadow: '0 4px 8px rgba(0,0,0,0.1)',background: "#fff", height: "95%" }}>
