...
const ListVouchers = () => {
  ...
  const openModal = (record: IVoucher) => {
    ...
    const initialFileList: UploadFile[] = record.image
      ? [{
          uid: "-1",
          name: "image.jpg",
          status: "done",
          url: `${BASE_URL}${record.image}`,
        }]
      : [];
...
const onFinish = (values: any) => {
  ...
  // Nếu người dùng đổi ảnh thì lấy file mới, còn không thì không gửi (giữ ảnh cũ trên server)
  ...
  const columns = [
    ...
    { title: "Mã", dataIndex: "ma", key: "ma", ...getColumnSearchProps("ma") },
    {
      ...
      render: (url: string) => (
        <Image
          src={url ? `${BASE_URL}${url}` : "https://via.placeholder.com/100"}
          width={100}
          height={100}
          style={{ objectFit: "cover", borderRadius: 8, boxShadow: "0 2px 8px rgba(0,0,0,0.15)" }}
    ...
    { title: "Trạng thái", dataIndex: "trang_thai", key: "trang_thai" },
    {
      ...
      render: (_: any, record: IVoucher) => (
        <Space>
          ...
          <Popconfirm title="Xác nhận xoá voucher này?" onConfirm={() => onDelete(record.id)}>
