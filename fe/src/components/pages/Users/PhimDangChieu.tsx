...
const PhimDangChieu = () => {
  const [PhimDangChieu, setMovies] = useState<any[]>([]);
  ...
  const fetchMovies = async () => {
    try {
      const res = await axios.get("http://127.0.0.1:8000/api/phim-dang-chieu");
      setMovies(res.data || []);
    } catch (error) {
      console.error("Lỗi khi fetch danh sách phim đang chiếu:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="home-wrapper">
      <div className="section section-trangcon">
        <h2 className="section-title-phimdangchieu">PHIM ĐANG CHIẾU</h2>
        {loading ? (
          <div
            style={{
              display: "flex",
